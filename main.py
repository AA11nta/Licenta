...
def new_alg(df):
    ...
    # Assuming the target has three classes, 0, 1, 2
    for i in range(3):
        fpr[i], tpr[i], _ = roc_curve(y_test, y_pred_probs[:, i],
                                      pos_label=i)
...
if choose_MecanismReteaNeuronala:
    ...
    with st.expander("Descrierea algoritmului"):
        st.write("- Rețelele neuronale reprezintă o componentă esențială a învățării automate și a analizei datelor, inspirate din structura și funcționarea creierului uman. Aceste rețele sunt capabile să învețe relații complexe și non-liniare din date, ceea ce le face extrem de puternice în diverse aplicații, de la recunoașterea imaginilor și procesarea limbajului natural la jocuri și vehicule autonome.")
